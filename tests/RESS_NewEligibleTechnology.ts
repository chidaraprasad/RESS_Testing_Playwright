...
test('RESS_Test1', async () => { 
  ...
  // Filtering the Code Tables to get the Eligible Technology table
  ...
  await page.getByRole('textbox', { name: 'Name contains specific text:' }).fill('RESS Eligible Technology');
  await page.getByRole('button', { name: 'Apply' }).click();
  await page.getByRole('button', { name: 'Apply' }).click();
